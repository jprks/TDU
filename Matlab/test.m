...
graphtemps(273,2500,101325);
end

function graphtemps(Ti, Tf, Pc)
T=[];Isp=[];
    for x = 1:(Tf-Ti)
        T(x) = Ti+(x-1);
        result = tdu(Ti+x, Pc);
        Isp(x) = cell2mat(result(25,2));
    end
    figure
    plot(T,Isp); 
    title(['Nitrogen, Chamber Pressure = ' num2str(Pc) ' Pa']);
    xlabel('Chamber Temp (K)'); ylabel('Specific Impulse (s)');
    
end
function result = tdu(Tc,Pc) % main function
%     clear all;
%     clc;
    %% universal constants
...
%%  === EDIT THESE ===
    ...
    specific_heat_ratio = 1.4013; % 1.4 for air
    molar_mass = .028013; % .0289645 for air
    molar_mass_units = '[kg/mol]';
    
    % chamber conditions
    chamber_temperature = Tc;
    temperature_units = '[K]';
    chamber_pressure = Pc; % 1.01325 bar = 1 atm
    pressure_units = '[Pa]';
